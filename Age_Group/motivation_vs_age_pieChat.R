...
bi_doption<-as.data.frame(read_excel("Demographic_and_BI_adoption.xlsx", sheet = "Business_intelligence_adoption", range = "F2:F13",col_names = FALSE,col_types = "text"))
...
tiff("motivation_vs_age_pieChat.tiff", units="in", width=5, height=5, res=300)
ggplot(data, aes(x="",y = ypos,fill=age)) + 
  geom_bar(stat="identity", width=1, color="white")+
  coord_polar("y", start=0)+
  labs(
    title = "Age Category vs Motivation",
    x="Age Categories",
    y="Frequency",
    fill ="Legend"
  )+theme_void()+
