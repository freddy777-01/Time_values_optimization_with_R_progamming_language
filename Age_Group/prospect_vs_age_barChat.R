...
bi_doption<-as.data.frame(read_excel("Demographic_and_BI_adoption.xlsx", sheet = "Business_intelligence_adoption", range = "H2:H13",col_names = FALSE,col_types = "text"))
...
tiff("prospect_vs_age_barChat.tiff", units="in", width=5, height=5, res=300)
ggplot(nFrame, aes(x=age,y = yes,fill=age)) + 
  geom_col(width = 0.5)+
  scale_y_continuous(breaks = seq(1,10,by=1))+
  coord_cartesian(ylim = c(1,10))+
  labs(
    title = "Age Category vs Prospect",
    x="Age Categories",
    y="Frequency",
    fill ="Legend"
  )+theme_minimal()
