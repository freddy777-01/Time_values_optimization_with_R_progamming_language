...
afternoon_predictedGA<-unlist(read_excel("Alldata_excel.xlsx", range = "M35:M59",col_names = FALSE))
...
midnightFrame <- data.frame(
  xAxis = seq(1,25),
  actual = afternoon_actual,
  predictedGA = afternoon_predictedGA,
  predictedMNLR = afternoon_predictedMNLR,
  predicredEncode = afternoon_predictredEncoding
)

tiff("afternoon1.tiff", units="in", width=10, height=10, res=300)
...
ggplot(midnightFrame,aes(x=xAxis))+
  ...
  labs(
    title ="Afternoon Load",
    y="Predicted Load",
    x ="",
    color="Legend"
  )+scale_y_continuous(breaks=seq(20,50,by=1))+
