...
library(dplyr)
#library(plotrix)
aca_qln<-as.data.frame(read_excel("Demographic_and_BI_adoption.xlsx", sheet = "Demography_information", range = "E2:E13",col_names = FALSE,col_types = "text"))
...
ggplot(data, aes(x="",y = prop,fill=qln)) + 
  ...
  geom_text(aes(label = qln), color = "white", size=2.5,position = position_stack(vjust = 0.5)) +
